...
public class Controller<T> {
  ...
  public Controller(final Service<T> service) {
    ...
    path(
        ...
        () -> {
          ...
          if (service.getDataType() == Alquiler.class) {
            get(
                "/canchastennisladrillo",
                ctx -> {
                  AlquilerService alquilerService = (AlquilerService) service;
                  response(ctx, alquilerService.getAlquileresCanchaTennisLadrillo());
                });
          }
          // Añadir al constructor de Controller.java, dentro del bloque path(...)
        ...
        writeText(jsonArray.toString(2), fileName);
          ...
          get(
            ...
            ctx -> {
              AlquilerService alquilerService = (AlquilerService) service;
              response(ctx, alquilerService.getAlquileresPiscinasOlimpicas());
            });
  ...
  public void info() {
    System.out
        .println(String.format("%s>> Creados los endpoints para %sService%s", Color.BLUE, currentPath, Color.RESET));
  }
}
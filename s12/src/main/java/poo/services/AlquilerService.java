...
public class AlquilerService implements Service<Alquiler> {
    ...
    private void verificarCruceHorarios(Alquiler nuevoAlquiler) throws Exception {
        ...
        for (Alquiler alquilerExistente : list) {
            // Si es el mismo alquiler (para casos de actualización) o no es la misma
            // instalación, no hay problema
    ...
    public JSONObject update(String id, String strJson) throws Exception {
        ...
        // La verificación ya se hizo en getUpdated, así que procedemos con la
        // actualización
    ...
    public Alquiler dataToAddOk(String strJson) throws Exception {
        ...
        // Si instalacionDeportiva es un string (ID), buscar el objeto instalación
        // completo
        ...
        // Construye el alquiler con el JSON completo (con objetos de socio e
        // instalación)
    ...
    // Modificar también el método getUpdated para incluir la verificación en
    // actualizaciones
    @Override
    public Alquiler getUpdated(JSONObject patch, Alquiler current) {
        ...
        if (patch.has("instalacionDeportiva")) {
            try {
                if (patch.get("instalacionDeportiva") instanceof String) {
                    ...
                    JSONObject jsonConInstalacionId = new JSONObject().put("instalacionDeportiva",
                            patch.getString("instalacionDeportiva"));
        ...
        try {
            // Verificar cruce de horarios (esta verificación se debe hacer antes de guardar
            // el alquiler)
            verificarCruceHorarios(a);
        } catch (Exception e) {
     ...
     * @throws Exception Si no se encuentra la instalación o si hay problemas con el
     *                   JSON
     */
    ...
    public JSONObject getAlquileresPiscinasOlimpicas() {
        try {
            // Cargamos la lista de alquileres si no está cargada
            if (list == null) {
                load();
            }

            // Crear una lista para almacenar los alquileres de piscinas olímpicas
            List<Alquiler> alquileresOlimpicos = new ArrayList<>();

            // Filtrar los alquileres que corresponden a piscinas olímpicas
            for (Alquiler alquiler : list) {
                InstalacionDeportiva instalacion = alquiler.getInstalacionDeportiva();

                // Verificar si la instalación es una piscina y si es olímpica
                if (instalacion instanceof Piscina && ((Piscina) instalacion).getOlimpica()) {
                    alquileresOlimpicos.add(alquiler);
                }
            }

            // Crear el array JSON con los datos de los alquileres
            JSONArray data = new JSONArray();
            for (Alquiler alquiler : alquileresOlimpicos) {
                data.put(alquiler.toJSONObject());
            }

            // Devolver el objeto JSON con el formato especificado
            return new JSONObject()
                    .put("data", data)
                    .put("size", alquileresOlimpicos.size())
                    .put("message", "Alquileres de piscinas olímpicas obtenidos con éxito");

        } catch (Exception e) {
            Utils.printStackTrace(e);
            return Utils.keyValueToJson(
                    "message", "error al obtener alquires de piscinas olimpicas",
                    "error", e.getMessage());
        }
    }

    public JSONObject getAlquileresCanchaTennisLadrillo() {
    try {
        // Cargamos la lista de alquileres si no está cargada
        ...
        // Crear un mapa para agrupar los alquileres por cancha
        Map<String, List<Alquiler>> alquileresPorCancha = new HashMap<>();
        Map<String, InstalacionDeportiva> canchaMap = new HashMap<>();
        
        // Filtrar los alquileres que corresponden a canchas de tenis de ladrillo
        for (Alquiler alquiler : list) {
            InstalacionDeportiva instalacion = alquiler.getInstalacionDeportiva();
            
            // Verificar si la instalación es una cancha de tenis de ladrillo
            if (instalacion instanceof CanchaTennis) {
                CanchaTennis cancha = (CanchaTennis) instalacion;
                if (cancha.getTipoCancha() == TipoCancha.LADRILLO) {
                    String canchaId = cancha.getId();
                    
                    // Añadir la cancha al mapa si no existe
                    if (!alquileresPorCancha.containsKey(canchaId)) {
                        alquileresPorCancha.put(canchaId, new ArrayList<>());
                        canchaMap.put(canchaId, cancha);
                    }
                    
                    // Añadir el alquiler a la lista de la cancha correspondiente
                    alquileresPorCancha.get(canchaId).add(alquiler);
                }
            }
        }
        
        // Crear el array JSON con los datos agrupados
        JSONArray data = new JSONArray();
        for (Map.Entry<String, List<Alquiler>> entry : alquileresPorCancha.entrySet()) {
            String canchaId = entry.getKey();
            List<Alquiler> alquileres = entry.getValue();
            
            // Obtener la información de la cancha
            InstalacionDeportiva cancha = canchaMap.get(canchaId);
            JSONObject canchaJSON = cancha.toJSONObject();
            
            // Crear JSONArray con los alquileres de esta cancha
            JSONArray alquileresJSON = new JSONArray();
            for (Alquiler alquiler : alquileres) {
                alquileresJSON.put(alquiler.toJSONObject());
            }
            
            // Crear objeto JSON para esta cancha y sus alquileres
            JSONObject canchaConAlquileres = new JSONObject()
                .put("cancha", canchaJSON)
                .put("alquileres", alquileresJSON);
            
            data.put(canchaConAlquileres);
        }
        
        // Contar el número total de canchas con alquileres
        int totalCanchas = alquileresPorCancha.size();
        
        // Crear el objeto JSON final con el formato especificado
        JSONObject resultJSON = new JSONObject()
                .put("data", data)
                .put("size", totalCanchas)
                .put("message", "Alquileres de canchas Tennis de Ladrillo obtenidos con éxito");
        
        // Exportar el resultado a un archivo JSON
        try {
            // Asegurarse de que la carpeta 'data' exista
            Utils.createFolderIfNotExist(Utils.PATH);
            
            // Crear un archivo para los resultados
            String fileName = Utils.PATH + "alquileres_canchas_tennis_ladrillo.json";
            
            // Usar el método writeText de Utils para escribir el JSON en el archivo
            Utils.writeText(resultJSON.toString(4), fileName);
            
            // Añadir información sobre el archivo exportado al mensaje
            resultJSON.put("file_path", fileName);
        } catch (Exception e) {
            // Si hay un error al escribir el archivo, agregarlo al JSON de respuesta
            resultJSON.put("file_error", "Error al escribir el archivo JSON: " + e.getMessage());
        }
        ...
        Utils.printStackTrace(e);
        return Utils.keyValueToJson(
                "message", "Error al obtener alquileres de canchas de tenis de ladrillo",
                "error", e.getMessage());
    }
}

    /*public JSONObject crearArchivo() {
        JSONObject insanity = getAlquileresCanchaTennisLadrillo();
        try {
            // Asegurarse de que la carpeta 'data' exista
            Utils.createFolderIfNotExist(Utils.PATH);

            // Crear un archivo para los resultados
            String fileName = Utils.PATH + "alquileresct-ladrillo.json";

            // Usar el método writeText de Utils para escribir el JSON en el archivo
            Utils.writeText(insanity.toString(4), fileName);

            // Añadir información sobre el archivo exportado al mensaje
            insanity.put("file_path", fileName);
        } catch (Exception e) {
            // Si hay un error al escribir el archivo, agregarlo al JSON de respuesta
            insanity.put("file_error", "Error al escribir el archivo JSON: " + e.getMessage());
        }

...
public class InstalacionDeportivaService implements Service<InstalacionDeportiva> {
    ...
    public JSONObject update(String id, String strJson) throws Exception {
        // Crear un JSONObject con las claves y valores a actualizar
        JSONObject newData = new JSONObject(strJson);
    
        // Encontrar la instalación deportiva a actualizar y recordar su posición
        InstalacionDeportiva instalacion = getItem(id);
        ...
        // Actualizar la instalación con los nuevos datos
        ...
        // Convertir la instalación actualizada a JSONObject
        JSONObject instalacionJson = instalacion.toJSONObject();
        
        // Actualizar la instalación deportiva en el archivo de alquileres
        try {
            // Obtener la ruta del archivo de alquileres
            String alquileresFile = Utils.getConfig("archivos").getString("alquileres");
            
            // Utilizar el método updateFile para actualizar referencias en el archivo de alquileres
            // Actualizar en campos "instalacion" e "instalacionDeportiva" si existen
            boolean updatedInstalacion = Utils.updateFile("instalacion", "id", id, instalacion.toJSONObject(), alquileresFile);
            boolean updatedDeportiva = Utils.updateFile("instalacionDeportiva", "id", id, instalacion.toJSONObject(), alquileresFile);
            
            if (Utils.trace) {
                if (updatedInstalacion || updatedDeportiva) {
                    System.out.println("Referencias actualizadas en alquileres para instalación ID: " + id);
                } else {
                    System.out.println("No se encontraron referencias a actualizar para instalación ID: " + id);
                }
            }
        } catch (Exception e) {
        ...
        // Actualizar el archivo JSON principal de instalaciones
        Utils.writeJSON(list, fileName);
    
        // Devolver la instalación actualizada
        return new JSONObject().put("message", "ok").put("data", instalacion.toJSONObject());
    }
